...
Component({
  ...
  methods: {
      ...
      wx.yx.chooseImage = ({ count = 3, success = '', fail = '', imgWidth = 1080, maxh2w = 8 }) => {
        ...
        wx.chooseImage({
          ...
          success(res) {
            ...
            const newImgArray = _this._packageImgsLocally(tempFiles);
    ...
    _packageImgsLocally(tempFiles) {
      const _this = this;
      const s = Date.now();
      const newImgArray = tempFiles.map((v, i) => {
        let format = '';
        const extensionArr = /\.([^.]*)$/.exec(v.path);
        if (extensionArr) {
          ...
          success(res) {
            ...
            for (let j = 0; j < imgLen; j++) {
              if (res.path === newImgArray[j].url) {
                ...
                const p = tmpH2W.indexOf('.');
                if (p >= 0) {
                  // 避免小数点过长 只保留小数点后四位
                  const sLen = tmpH2W.length;
                  const diff = sLen - p;
                  if (diff > 5) {
    ...
    _compress(imgs) {
      ...
      var c = function () {
        ...
        // 开始画图
        const h = Math.round(w * imgObj.h2w);
        let quality = 0.8;
