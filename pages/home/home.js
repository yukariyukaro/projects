...
Page({
  ...
  getPost: function () {
    var that = this
    newRequest("/post/list/all", {page: that.data.page}, that.getPost)
    .then((res) => {
      if(res.code == 200){
  ...
  getPostByHot: function () {
    var that = this
    newRequest('/post/list/hot', {page: that.data.page}, that.getPostByHot)
    .then((res) => {
      wx.hideLoading()
      if(res.code == 200){
  ...
  getPostByLatest: function () {
    var that = this
    newRequest('/post/list/latest', {page:that.data.page}, that.getPostByLatest)
    .then((res) => {
      wx.hideLoading()
      if(res.code == 200){
  ...
  getPostByTopic: function () {
    ...
    newRequest("/post/list/topic", {
      ...
      page: that.data.page}, that.getPostByTopic)
      .then((res) => {
        if(res.code == 200){
          if(wx.getStorageSync('ban_uni_post')){
            var post_list = res.one_list.filter(function (item) {
