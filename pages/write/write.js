...
Page({
  ...
  switchPublicChange: function (e) {
    this.setData({
      post_public: e.detail.value
    });
  },
  // 选择是否带编号
  switchSerialChange: function (e) {
    this.setData({
      post_with_serial: e.detail.value
    });
  },
  switchUNIChange: function (e) {
    this.setData({
      post_is_uni: e.detail.value
    });
  },
  switchMultiChange: function (e) {
    this.setData({
      vote_is_multi: e.detail.value
    });
  },
      ...
      newRequest("/post/single/post", {
        ...
        if (res.code == 200) {
          if (post_public == 1) {
            ...
            setTimeout(() =>
              wx.reLaunch({
                url: '/pages/home/home?from=write',
              }),
              1000);
          }
          if (post_public == 2) {
